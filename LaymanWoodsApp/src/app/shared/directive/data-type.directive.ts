...
import { dataTypeEnum } from '../enums/app.enums';
        ...
        if (this.dataType && c.value) {
            if (!this.isValid(c.value)) {
                ...
                if (Object.keys(errormsg).length === 0) {
                    errormsg = c.errors;
                    let typeerror = this.errorMessage(this.dataType);
                    typeerror = typeerror != null ? Object.keys(typeerror)[0] : '';
                    for (const keymsg in errormsg) {
                        if (keymsg === typeerror) {
                            errormsg = {};
